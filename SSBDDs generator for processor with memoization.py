...
class matrix(object):
    def __init__(self, size, pattern):
        ...
        for row_position in range(1, row):
            for col_position in range(1, col):
                self.map = np.concatenate(
                    [[row_position], [col_position]])  # Concatenate the index together to get indices
        ...
        self.lis_memo = {}  # Remember previous result if same inputs occur again
                    ...
                    if (i != j):
                        if (i == m or j == m):
                            ...
                            if i or j not in self.val_memo:
                                ...
                                return self.lis_memo
                            for el in self.lis_memo:
                                self.elem = self.lis_memo[el]
